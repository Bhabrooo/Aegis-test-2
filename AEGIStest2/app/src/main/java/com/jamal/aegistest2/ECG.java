...
public class ECG {
    //constants
    private static final int fs = 250;// same as sample_rate. Shortened for ease
    private static final int sample_rate = 250;
    private static final int rawL = 1500;
    private static final int sqrd_dL = 1504;
    private static final int integralL = 1541;

    //variables
    private double mean_RR, m_selected_RR, test_m;
    public double avg_RR_samp,avg_RR_msec, BPM;
    private double raw[], sqrd_d[], integral[];
    private int delay, not_nois, skip, ser_back;
    public Vector<Double> qrs_c ;
    public Vector<Integer> qrs_i;
    ...
    // fucntions
    // loads the entire length vector into the raw array
    void load_data(Vector<Double> ecg_ble){
        for (int i=0; i<integralL; i++){
            this.integral[i]=0;
            if(i<sqrd_dL)
            {
    ...
    void update_data(Vector<Double> ecg_ble){
        this.delay = 0;
        this.mean_RR = 0;
        this.m_selected_RR = 0;
        this.test_m = 0;
    ...
    void integrate(){
        delay = delay + 15;
        double filt_inti[]= new double[38];
        for (int i=0;i<38;i++){
            filt_inti[i]=0.02632;
        }
        int l_filt=38;
    ...
    void r_peaks(){
        ...
        locs = peakfind_loc(integral,50);
        pks = peakfind_amp(integral,50);

        // --------------------------------------------
        // initialize the training phase of 2 secs
        double THR_SIG, THR_NOISE, SIG_LEV, NOISE_LEV;
        THR_SIG = findmax1(integral,1,500);
        THR_SIG = THR_SIG/3;
        THR_NOISE = findmean(integral,1,500);
        ...
        THR_SIG1 = findmax1(raw,1,500);
        THR_SIG1 = THR_SIG1/3;
        THR_NOISE1 = findmean(raw,1,500);
        ...
        for(int i=0; i<pks.size();i++){
            ...
            if( ((locs.elementAt(i)-38)>= 1) && (locs.elementAt(i)<= 1500) ){
                x_i=findmax2(raw, (locs.elementAt(i)-38), locs.elementAt(i));
                y_i=findmax1(raw, (locs.elementAt(i)-38), locs.elementAt(i));
            }
            else if(i==0){
            ...
            else if(locs.elementAt(i)>=1500){
                x_i=findmax2(raw, (locs.elementAt(i)-38), 1500);
                y_i=findmax1(raw, (locs.elementAt(i)-38), 1500);
            }
            ...
            if(test_m!=0){
                ...
                if( (locs.elementAt(i)-qrs_i.elementAt(qrs_i.size()-1)) >= (1.66*test_m ) ){
                    ...
                    double pks_temp;
                    pks_temp = findmax1(integral, (50 + qrs_i.elementAt(qrs_i.size() - 1)), (locs.elementAt(i) - 50));
                    int locs_temp;
                    locs_temp = findmax2(integral, (50+qrs_i.elementAt(qrs_i.size()-1)), (locs.elementAt(i)-50));
                    locs_temp = qrs_i.elementAt(qrs_i.size()-1) + 50 + locs_temp -1;
                    if(pks_temp > THR_NOISE){
                        ...
                        double y_i_t;
                        if(locs_temp <= 1500){
                            x_i_t=findmax2(raw, (locs_temp-38), locs_temp);
                            y_i_t=findmax1(raw, (locs_temp-38), locs_temp);
                        }
                        else{
                            x_i_t=findmax2(raw, (locs_temp-38), 1500);
                            y_i_t=findmax1(raw, (locs_temp-38), 1500);
                        }
                        //take care of bandpass signal threshold
                        if(y_i_t > THR_NOISE1){
                            int pushed = locs_temp-38+(x_i_t-1);
            ...
            if(pks.elementAt(i) >= THR_SIG){
                ...
                if(qrs_c.size()>=3){
                    if((locs.elementAt(i)-qrs_i.elementAt(qrs_i.size()-1)) <= 90){
                        Vector<Integer> temp1 = new Vector<Integer>(0);
                        Vector<Integer> temp2 = new Vector<Integer>(0);
                        double slope1, slope2;

                        finddiff2( temp1, integral, (locs.elementAt(i)-19), locs.elementAt(i) );
                        slope1=findmean_v(temp1, 1, temp1.size()); // mean slope of the waveform at that position

                        finddiff2(temp2, integral, (qrs_i.elementAt(qrs_i.size()-1) -19), qrs_i.size());
                ...
                if(skip==0){
                    ...
                    if(y_i >= THR_SIG1){
                        if(ser_back!=0){
                            ...
                            int pushed2;
                            pushed2= locs.elementAt(i)-38+(x_i-1);
    ...
    void get_RR_BPM(){
        Vector<Integer> temp_diffRR= new Vector<Integer>(0); //create temporary diffRR
        finddiff(temp_diffRR, qrs_i, 1, qrs_i.size());  //calculate the RR intervals for all peaks
        avg_RR_samp=findmean_v(temp_diffRR,1,temp_diffRR.size());  //find avg RR interval in samples
        avg_RR_msec=(avg_RR_samp*1000/fs);   // get RR interval in milliseconds
        BPM=60*(1000/avg_RR_msec);
    }
// CLASS END
}
